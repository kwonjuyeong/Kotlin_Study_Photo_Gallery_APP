...
다른 파서와 데이터 형식
- Gson은 널리 사용되는 JSON 파서이지만, 다른 파서도 있다. 예를 들어 Square 사의 Moshi 라이브러리 등이 있다.

요청 취소하기
- 웹 요청이 계속 실행되면 장치의 배터리 소모나 CPU의 불필요한 사용, 사요자의 데이터 사용량이 증가할 수 있다.
- 앱이 시작될 때 사용자가 곧바로 백 버튼을 누르더라도 웹 요청이 계속 실행되는데 이럴 때 웹 요청의 결과를 무시하는 것보다는 장치의 어딘가에 저장해두면 좋다.
- 현재는 요청 결과를 저장하지 않기 떄문에 .cancel() 를 호출해서 웹 요청을 취소하면 된다.
 */